...
def extract_patches(filename, tup, args):
    ...
    for p, c in zip(points, clusters):
        ...
        # this will create if not exist else append
        if c not in cluster_h5_files:
            cluster_h5_files[c] = h5py.File(h5_file_path,'a')
        
        
        img_filename = f'{c}_{os.path.splitext(os.path.basename(filename))[0]}_{count}'
        
        if img_filename in cluster_h5_files[c]:
            logging.warning(f'image file {img_filename} already exist in {h5_file_path}, skipping...')
            continue
        
        cluster_h5_files[c].create_dataset(img_filename,data=roi,compression="gzip")
    ...
    for h5f in cluster_h5_files.values():
        h5f.close()
...
if __name__ == "__main__":
    ...
    results = []
    results = Parallel(n_jobs=num_cores, verbose=10)(delayed(calc_features)(f, args) for f in files[:5])
    ...
    '''
    #print("patches{}".format(patches_files))
    #print("shape:{}".format(len(patches_files)))
